...
\anote
对于任意一个复数 $z=a+ib$，可以用复平面上的一个点来表示，也可以用极坐标来表示：
...
\begin{center}
	\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=0.8cm,y=0.8cm]
		\begin{axis}[
			...
			\draw [->,line width=1.5pt] (0,0) -- (10.5,0);
			\draw [->,line width=1.5pt] (0,0) -- (0,10.5);
			\draw (0.3,6) node[anchor=north west] {\LARGE Imaginary part as YAxis};
			\draw (3.2,0.9) node[anchor=north west] {\LARGE Real part as XAxis};
			\draw [line width=1.5pt] (0,0)-- (8,4);
			\draw (8.7,4.45) node[anchor=north west] {\LARGE(x,y)};
			\begin{scriptsize}
				\draw [fill=black] (0,0) circle (3pt);
				\draw[color=black] (10,0.8) node {\LARGE Re};
				\draw[color=black] (0.7,10) node {\LARGE Im};
